...
{% macro default__first_value(first_value_field, partition_field, order_by_field, order="asc")  %}
    ...
    first_value( {{ first_value_field }} ) over (partition by {{ partition_field }} order by {{ order_by_field }} {{ order }} )
...
{% macro redshift__first_value(first_value_field, partition_field, order_by_field, order="asc") %}
    ...
    first_value( {{ first_value_field }} ) over (partition by {{ partition_field }} order by {{ order_by_field }} {{ order }} , {{ partition_field }} rows unbounded preceding )
